...
from .services.ocr import OCRUnavailableError, extract_panel_texts, extract_text, recover_expected_text
...
def run_review(image_bytes: bytes, application: ApplicationData) -> ReviewResponse:
    ...
    alternate_image_lookup = dict(alternate_images)
    recovery_image = alternate_image_lookup.get("boosted grayscale", processed_image)
    recovery_candidates = []
    if application.class_type:
        recovery_candidates.append(f"{application.brand_name} {application.class_type}")
    ...
    recovered_text = recover_expected_text(
        ocr_result.text,
        recovery_candidates,
        supplemental_texts=extract_panel_texts(recovery_image),
    )
    ...
    if recovered_text:
        preprocessing_notes.append(
            "Recovered likely text from OCR fragments and panel crops."
        )
